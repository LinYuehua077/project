...
def processMonthlyData(file_path_lst, year, month, data_type, file_type='parquet',include_previous_last_day=True):
    """
    处理指定月份的数据，可选择是否包含上个月最后交易日的数据
    ...
    if monthly_data:
        ...
        print(f"警告4: {year}年{month}月没有找到数据")
        return None
