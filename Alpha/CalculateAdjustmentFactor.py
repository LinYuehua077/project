...
if 'adjustment_temp.parquet' not in os.listdir('./'):
    ...
    df.to_parquet('./adjustment_temp.parquet')


# 现在已经有中间表格了
df = pd.read_parquet('./adjustment_temp.parquet')

# 使用开盘价计算复权因子
df['adjust_factor_open'] = df['openw'] / df['open']


# 使用收盘价计算复权因子
df['adjust_factor_close'] = df['close'] / df['close']

# 平均复权因子
df['avg_adjust_factor'] = (df['adjust_factor_open'] + df['adjust_factor_close']) / 2

# 保存计算好的复权因子
df.to_parquet('./adjustment.parquet')